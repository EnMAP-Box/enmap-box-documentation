...
Saves a `raster layer <https://enmap-box.readthedocs.io/en/latest/general/glossary.html#term-raster-layer>`_ as a GeoTiff, ENVI or VRT file. This is a slimmed down version of the more powerful "Translate raster `layer <https://enmap-box.readthedocs.io/en/latest/general/glossary.html#term-layer>`_" algorithm. In addition, it also accepts non-GDAL layer.
...
:guilabel:`Raster I/O mode` [enum]
    Choose how raster data is read and written during export.
    Default: *0*
...
``>qgis_process help enmapbox:saverasterlayeras``::
    ...
    mode: Raster I/O mode (optional)
        Default value:    0
        Argument type:    enum
        Available values:
            - 0: Auto (recommended)
            - 1: GDAL (gdal.Translate)
            - 2: QGIS (QgsRasterDataProvider.block)
        Acceptable values:
